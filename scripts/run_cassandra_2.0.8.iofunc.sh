...
export CODECLASSPATH=$CODEROOT:$CODEROOT/callGraphAnalysis:$CODEROOT/datastruct:$CODEROOT/falsePositivePatterns:$CODEROOT/findAllJavaAppIOClass:$CODEROOT/infiniteLoopDetection:$CODEROOT/truePositivePattern:$CODEROOT/utilities:$CODEROOT/soot/coffi:$CODEROOT/truePositivePatterns
...
do
    ...
    java -cp .:$CODECLASSPATH:$CLASSPATH findMethodsinAbsInters -cp .:$CASSANDRACLASSPATH:$CLASSPATH -w -p cg enabled:true,jdkver:7,all-reachable:true,implicit-entry:false,safe-newinstance:false,verbose:false,trim-clinit:true -p jb enabled:true -p wjtp enabled:true ${line} 1> /home/ting/DataLoopBugDetection/ResultIOClasses/cassandra.io.func/abstractClasses/${line}.txt 2> logs/cassandra.208.log
    test $? -gt 128 && break; #use Ctrl-C to break the loop
    (( linenum++ ))
done < ${allClassFile}
##############################